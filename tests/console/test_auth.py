...
class TestConsoleAuth:
    ...
    def _mock_env(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """DEV_MODE fixture — requires ALLOW_DEV_MODE=1 ack and localhost bind
        after the v0.6 C4 hardening."""
        monkeypatch.setenv("GOVERNANCE_CONSOLE_DEV_MODE", "true")
        monkeypatch.setenv("GOVERNANCE_CONSOLE_ALLOW_DEV_MODE", "1")
        monkeypatch.setenv("GOVERNANCE_CONSOLE_HOST", "127.0.0.1")
        monkeypatch.setenv("GOVERNANCE_DATABASE_URL", "postgresql://x:x@localhost/x")
        monkeypatch.setenv("GOVERNANCE_AUDIT_SECRET", "x" * 64)

    @pytest.fixture
    def _mock_env_prod(self, monkeypatch: pytest.MonkeyPatch) -> None:
        """Production-mode fixture — no DEV_MODE, just the required DB + audit vars.
        Used by tests that exercise the app without the dev-mode admin grant."""
        monkeypatch.delenv("GOVERNANCE_CONSOLE_DEV_MODE", raising=False)
        monkeypatch.delenv("GOVERNANCE_CONSOLE_ALLOW_DEV_MODE", raising=False)
        monkeypatch.setenv("GOVERNANCE_CONSOLE_HOST", "127.0.0.1")
        monkeypatch.setenv("GOVERNANCE_DATABASE_URL", "postgresql://x:x@localhost/x")
        monkeypatch.setenv("GOVERNANCE_AUDIT_SECRET", "x" * 64)
    ...
    def test_dev_mode_allows_unauthenticated_access(self, _mock_env: None) -> None:
        ...
        # Reload the module to pick up env changes (DEV_MODE is captured at import time)
        import importlib
        import codeatelier_governance.console.app as app_mod
        importlib.reload(app_mod)
        assert app_mod.DEV_MODE is True

    def test_health_endpoint_is_unauthenticated(self, _mock_env_prod: None) -> None:
        """The /api/health endpoint should never require auth.

        Runs in production-mode (DEV_MODE unset) to prove that even without
        the dev-mode admin grant, the health endpoint is reachable. Module
        must be reloaded after the env fixture so the module-level DATABASE_URL
        constant picks up the patched value (it's read via os.environ.get()
        ...
        # Health endpoint works regardless of auth state. The TestClient
        # context manager runs the FastAPI lifespan, which in v0.6 runs the
        # C4 dev-mode guards — DEV_MODE is False here so they're a no-op.
